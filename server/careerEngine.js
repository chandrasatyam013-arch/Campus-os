...
export class CareerEngine {
     ...
     */
    static calculateMatchScore(userTraits, careerTraits) {
     ...
     */
    static calculateCareerFit(profile) {
        const results = CAREER_TEMPLATES.map(career => {
            ...
            // Weights
            const totalScore = ((interestMatch * 25) +
                (skillMatch * 25) +
                (subjectMatch * 15) +
                (styleMatch * 15) +
                (prefMatch * 10) +
            ...
            if (finalScore < 20)
                finalScore = Math.floor(Math.random() * 10) + 20; // pseudo random floor just so it doesn't say 0%
            let rawExplanation = `Your strongest alignment comes from:\n`;
            if (interestMatch > 0.3)
                rawExplanation += `✓ Strong interest alignment\n`;
            if (skillMatch > 0.3)
                rawExplanation += `✓ Strengths match required skills\n`;
            if (subjectMatch > 0.3)
                rawExplanation += `✓ Preferred subjects are highly relevant\n`;
            if (styleMatch > 0.3)
                rawExplanation += `✓ Compatible work style preferences\n`;
            return {
                career: career.name,
                careerId: career.id,
     ...
     */
    static generateRoadmap(careerName, subjects, marks) {
        ...
        for (const sub of subjects) {
            ...
            if (subjectMarks.length > 0) {
                ...
                if (percentage >= 80) {
                    ...
                    if (sub.name.toLowerCase().includes('math'))
                        strongTopics.add('math');
                    if (sub.name.toLowerCase().includes('python'))
                        strongTopics.add('python');
                    if (sub.name.toLowerCase().includes('java') || sub.name.toLowerCase().includes('c++'))
                        strongTopics.add('programming');
                }
            }
        }
