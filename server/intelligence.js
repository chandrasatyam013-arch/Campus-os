export class IntelligenceEngine {
    ...
    static calculateSubjectAttendance(subject, records, defaultTarget = 85, defaultMin = 75) {
    ...
    static calculateOverallAttendance(subjects, records, settings) {
        ...
        const intelList = subjects.map(s => this.calculateSubjectAttendance(s, records, settings.attendanceTarget, settings.minimumAttendance));
    ...
    static calculateSubjectPerformance(subject, marks) {
        ...
        if (subjectMarks.length >= 2) {
            ...
            if (trendPercentageChange >= 3.0)
                trend = 'IMPROVING';
            else if (trendPercentageChange <= -3.0)
                trend = 'DECLINING';
            else
                trend = 'STABLE';
        }
        ...
        if (averagePercentage >= 90) {
            gradeEstimate = 'O';
            gradePointEstimate = 10;
        }
        else if (averagePercentage >= 80) {
            gradeEstimate = 'A+';
            gradePointEstimate = 9;
        }
        else if (averagePercentage >= 70) {
            gradeEstimate = 'A';
            gradePointEstimate = 8;
        }
        else if (averagePercentage >= 60) {
            gradeEstimate = 'B+';
            gradePointEstimate = 7;
        }
        else if (averagePercentage >= 50) {
            gradeEstimate = 'B';
            gradePointEstimate = 6;
        }
        else if (averagePercentage >= 40) {
            gradeEstimate = 'C';
            gradePointEstimate = 5;
        }
        else {
            gradeEstimate = 'F';
            gradePointEstimate = 0;
        }
        return {
            subjectId: subject.id,
            subjectName: subject.name,
    ...
    static calculateAllSubjectPerformances(subjects, marks) {
    ...
    static calculateSGPA(performances) {
    ...
    static calculateAssignmentUrgency(assignment, subjectIntel) {
    ...
    static generateRecommendations(subjects, attendanceRecords, marks, assignments, timetable, events, settings) {
    ...
    static generateWhatShouldIDoToday(recommendations, timetable, subjects) {
        const now = new Date();
        const currentDay = ['SUNDAY', 'MONDAY', 'TUESDAY', 'WEDNESDAY', 'THURSDAY', 'FRIDAY', 'SATURDAY'][now.getDay()];
    ...
    static generateWeeklySummary(subjects, records, marks, assignments, events) {
        ...
        let attendanceDirection = 'STABLE';
        if (pastWeekRecords.length > 0 && priorWeekRecords.length > 0) {
            ...
            if (attendanceChangePercent > 0.5)
                attendanceDirection = 'UP';
            else if (attendanceChangePercent < -0.5)
